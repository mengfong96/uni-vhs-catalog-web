...
<head>
  ...
  <title>Order Details | Lee's VHS Movies Ordering System</title>
  <!-- Bootstrap -->
...
<body>
  ...
  <?php
    try {
      ...
      $stmt = $conn->prepare("SELECT * FROM tbl_orders_a161103, tbl_staffs_a161103_pt2, tbl_customers_a161103_pt2 WHERE tbl_orders_a161103.fld_staff_num = tbl_staffs_a161103_pt2.fld_staff_num AND tbl_orders_a161103.fld_customer_num = tbl_customers_a161103_pt2.fld_customer_num AND fld_order_num = :oid");
  ...
  <div class="container-fluid">
    ...
    <div class="row">
      ...
      <form action="orders_details.php" method="post" class="form-horizontal" name="frmorder" id="forder">
        <div class="form-group">
          ...
          <div class="col-sm-9">
            <select name="pid" class="form-control" id="prd">
              ...
              ?>
                <option value="<?php echo $productrow['fld_product_id']; ?>"><?php echo $productrow['fld_product_name']." | ".$productrow['fld_price']; ?></option>
              <?php
              }
              $conn = null;
              ?>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-9">
            <input name="quantity" type="number" class="form-control" id="qty" min="1" max="10">
          </div>
        </div>
      <div class="form-group">
    ...
    <div class="row">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_orders_details_a161103,
            tbl_products_a161103_pt2 WHERE
            tbl_orders_details_a161103.fld_product_id = tbl_products_a161103_pt2.fld_product_id AND fld_order_num = :oid");
          $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
          $oid = $_GET['oid'];
        $stmt->execute();
        $result = $stmt->fetchAll();
